...
design_final <- gsub("NA", "", design_final)
...
#Question: It's not considering the isogenic replicates, only technical, should we discriminate that? (10 replicates mean 5 technical replicates of 2 distinct isogenic samples)
...
#I want to exclude this ENCSR842VGU
# maybe exclude these: ENCFF918EVM, ENCFF741BSF, ENCFF255JSS (failed, didnt match with replicates, same input control); ENCFF962AHN, ENCFF044YWU,ENCFF794RCZ, ENCFF523UYJ, ENCFF205RPZ, ENCFF435KUB, ENCFF632WAT, ENCFF254TVF (paired, read 2)
#change ENCFF886BMC (read2) for ENCFF633HXG (read1) at control ENCSR689CZH

```{r}
# I didn't use this
##control2exp <- fastq_info_merged %>% 
  ungroup() %>% 
...
fastq_info_merged <- merge(fastq_info_merged, control2exp, all.x = T)
fastq_info_merged[is.na(fastq_info_merged$controlled_by), "group"] <- fastq_info_merged[is.na(fastq_info_merged$controlled_by), "experiment_accession"]
###until here
```
