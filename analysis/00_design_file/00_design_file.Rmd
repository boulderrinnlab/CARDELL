...
# Url to retrieve all the samples from encode
...
# We are getting some biosample informatio to maybe merge on later
...
#TODO: check to see if the controls are comming down from encode API
```{r}
fastq_info_merged$fastq_1 <- paste0("../../data/chipseq_fastq/", fastq_info_merged$file_accession, ".fastq.gz")
fastq_info_merged$Controls <- gsub("/", "", gsub("/experiments/", "", fastq_info_merged$controlled_by))
...
contrls <- fastq_info_merged %>% filter(is.na(Controls))
